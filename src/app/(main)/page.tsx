            ...
            <section className="py-24 bg-muted/30">
                <div className="container mx-auto px-4 md:px-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        {[
                            { title: "Environmental Clearance", img: "https://parivesh.nic.in/assests/img/gc1.jpg" },
                            { title: "Forest Clearance", img: "https://parivesh.nic.in/assests/img/gc2.png" },
                            { title: "Wild Life Clearance", img: "https://parivesh.nic.in/assests/img/gc3.png" },
                            { title: "CRZ Clearance", img: "https://parivesh.nic.in/assests/img/gc4.png" },
                        ].map((item, i) => (
                            <div key={i} className="group relative aspect-[4/3] overflow-hidden rounded-3xl shadow-xl transition-all hover:-translate-y-2">
                                <img src={item.img} alt={item.title} className="absolute inset-0 h-full w-full object-cover group-hover:scale-110 transition-transform duration-500" />
                ...
                <div className="container mx-auto px-4 md:px-6 flex items-center gap-12 grayscale opacity-50 hover:grayscale-0 hover:opacity-100 transition-all">
                    <div className="flex gap-16 animate-infinite-scroll">
                        {['dot', 'cpcb', 'ministryofpower', 'bharatkosh', 'idea4life', 'qci', 'nsws'].map((logo, i) => (
                            <img key={i} src={`https://parivesh.nic.in/assests/img/${logo}${logo === 'idea4life' ? '.png' : logo === 'qci' ? '.png' : logo === 'nsws' ? '.png' : '.svg'}`} alt={logo} className="h-12 w-auto object-contain shrink-0" />
                        ))}
                    </div>
                    {/* Same set for infinite loop */}
                    <div className="flex gap-16 animate-infinite-scroll">
                        {['dot', 'cpcb', 'ministryofpower', 'bharatkosh', 'idea4life', 'qci', 'nsws'].map((logo, i) => (
                            <img key={i + 10} src={`https://parivesh.nic.in/assests/img/${logo}${logo === 'idea4life' ? '.png' : logo === 'qci' ? '.png' : logo === 'nsws' ? '.png' : '.svg'}`} alt={logo} className="h-12 w-auto object-contain shrink-0" />
                        ))}
                    </div>
                </div>
            </section>
